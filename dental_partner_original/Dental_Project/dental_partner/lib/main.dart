...
void main() async {
  WidgetsFlutterBinding.ensureInitialized(); // 플러터 엔진 준비

  await Supabase.initialize(
    url: 'https://rcpmdwvzyfwwlpagetyn.supabase.co', // 아까 찾은 Project URL
    anonKey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJjcG1kd3Z6eWZ3d2xwYWdldHluIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ2MjUzODcsImV4cCI6MjA5MDIwMTM4N30.sEJ50EkwKLo5P8nmTxJE82vmtzcTCzGHljOSVJDBU7Q',       // 아까 찾은 anon public 키
  );
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (authRes.user == null) {
        _showErrorDialog("로그인 실패", "이메일 또는 비밀번호가 일치하지 않습니다.");
        return;
      }

      // 2. hospitals 테이블에서 status 확인
      ...
      if (data == null) {
        ...
        _showErrorDialog("오류", "등록된 병원 정보를 찾을 수 없습니다.");
        return;
      }

      if (data['status'] == 'pending') {
        await supabase.auth.signOut();
        _showErrorDialog("승인 대기", "관리자 승인 후 로그인이 가능합니다.");
        return;
      }
      ...
      _showErrorDialog("오류 발생", "연결에 실패했습니다: $e");
    } finally {
...
class _SignUpPageState extends State<SignUpPage> {
  ...
  void _searchAddress() async {
    // 1. 주소 검색 화면으로 이동 (결과를 기다림)
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => const AddressSearchScreen()),
    );

    // 2. 결과값이 유효한 String인지 엄격하게 체크
    if (result != null && result is String && result.isNotEmpty) {
      if (!mounted) return;
      setState(() {
        // 주소 입력 컨트롤러에 텍스트를 넣어줍니다.
        // (기존에 _addressCtrl을 쓰고 계셨을 거예요)
        _addressCtrl.text = result;
      });

      // 주소가 들어왔으니 다음 입력창(상세주소 등)으로 포커스를 옮겨주면 센스 만점!
      FocusScope.of(context).unfocus();
    } else {
      // 혹시라도 결과가 이상하게 왔을 때를 대비한 로그
      print("주소 검색 결과가 올바르지 않습니다: $result");
    }
  }
  ...
  Future<void> _signUp() async {
    ...
    if (_nameCtrl.text.trim().isEmpty) {
      _showErrorDialog('입력 확인', '병원명을 입력해주세요.'); return;
    }
    if (_addressCtrl.text.trim().isEmpty) {
      _showErrorDialog('입력 확인', '병원 주소를 입력해주세요.'); return;
    }
    if (_emailCtrl.text.trim().isEmpty) {
      _showErrorDialog('입력 확인', '이메일을 입력해주세요.'); return;
    }
    if (_pwCtrl.text.isEmpty) {
      _showErrorDialog('입력 확인', '비밀번호를 입력해주세요.'); return;
    }
    if (_pwCtrl.text.length < 6) {
      _showErrorDialog('입력 확인', '비밀번호를 6자리 이상 입력해주세요.'); return;
    }
    if (_pwCtrl.text != _pwCheckCtrl.text) {
      _showErrorDialog('가입 실패', '비밀번호가 일치하지 않습니다.'); return;
    }
    ...
    try {
      ...
      Future<String?> uploadFile(PlatformFile? file, String folder) async {
        ...
        final path = '$folder/${uid}_${file.name}';
        String mime = 'application/octet-stream';
        final ext = file.name.split('.').last.toLowerCase();
        if (ext == 'pdf') mime = 'application/pdf';
        else if (ext == 'png') mime = 'image/png';
        else if (ext == 'jpg' || ext == 'jpeg') mime = 'image/jpeg';
        await bucket.uploadBinary(path, file.bytes!, fileOptions: FileOptions(contentType: mime));
        return bucket.getPublicUrl(path);
      }

      final licenseUrl  = await uploadFile(licenseFile,  'license');
      final businessUrl = await uploadFile(businessFile, 'business');
      final reportUrl   = await uploadFile(reportFile,   'report');

      // ── 4. hospitals 테이블에 병원 정보 insert ──────
      await Supabase.instance.client.from('hospitals').insert({
        'ykiho':   uid,                         // 임시 ykiho (admin이 실제 코드로 교체)
      ...
      // ── 5. hospital_documents 테이블에 파일 URL 저장 ─
      for (final entry in [
      ...
      // 가입 직후 자동 로그인 세션 종료 (승인 전까지 사용 불가)
      ...
      _showErrorDialog(
        '파일 업로드 실패',
        'Supabase Storage에 hospital-documents 버킷이 생성되어 있는지 확인해주세요.\n\n오류: ${e.message}',
      );
    } catch (e) {
      _showErrorDialog('가입 실패', '알 수 없는 오류가 발생했습니다.\n$e');
    } finally {
...
class _AddressSearchScreenState extends State<AddressSearchScreen> {
  ...
  Future<void> _startServer() async {
    try {
      ...
      // ✅ 핵심 수정: 채널 이름을 패키지가 실제로 쓰는 'DaumPostcodeResult'로 변경
      await controller.addJavaScriptChannel(
        'DaumPostcodeResult',
        onMessageReceived: (JavaScriptMessage message) {
          try {
            final result = DaumPostcodeCallbackParser.fromPostMessage(message.message);
            if (result != null && result.roadAddress.isNotEmpty) {
              final String finalAddress = result.roadAddress.toString();
              print('선택된 주소: $finalAddress');
              Future.delayed(const Duration(milliseconds: 100), () {
                if (mounted) Navigator.of(context).pop(finalAddress);
              });
            }
          } catch (e) {
            print('주소 파싱 에러: $e');
            // 파싱 실패 시 raw JSON에서 직접 꺼내기 시도
            try {
              final decoded = jsonDecode(message.message);
              final address = decoded['roadAddress'] ?? decoded['jibunAddress'] ?? '';
              if (address.toString().isNotEmpty && mounted) {
                Future.delayed(const Duration(milliseconds: 100), () {
                  if (mounted) Navigator.of(context).pop(address.toString());
                });
              }
            } catch (_) {}
          }
...
class _HospitalInfoPageState extends State<HospitalInfoPage> {
  ...
  bool _isLoading = true;
  String? _currentYkiho; // 현재 로그인한 병원의 식별자
  ...
  void initState() {
    super.initState();
    _loadInitialData(); //추가: 페이지 켜지자마자 실행
  }

  // [실제 구현] Supabase에서 모든 데이터를 긁어오는 함수
  Future<void> _loadInitialData() async {
    try {
      final user = supabase.auth.currentUser;
      if (user == null) return;

      // 1. hospitals 테이블에서 내 이메일과 일치하는 병원 정보 가져오기
      ...
      if (hData != null) {
        _currentYkiho = hData['ykiho']; // ykiho 저장

        // 2. 의료진(doctors) 정보 가져오기
        final dData = await supabase
            .from('doctors')
            .select()
            .eq('ykiho', _currentYkiho!);

        // 3. 가격(hospital_prices) 정보 가져오기
        ...
        setState(() {
          // 기본 정보 세팅
          hospitalBaseInfo['name'] = hData['yadm_nm'] ?? '';
          hospitalBaseInfo['address'] = hData['addr'] ?? '';
          hospitalBaseInfo['operatingHours'] = hData['operating_hours'] ?? {};

          // 의료진 세팅
          doctors = List<Map<String, String>>.from(dData.map((d) => {
            'name': d['name'].toString(),
            'special': d['specialty'] ?? '',
            'year': d['started_year']?.toString() ?? '2024',
            'month': d['started_month']?.toString().padLeft(2, '0') ?? '01',
          }));

          // 가격 세팅
          products = List<Map<String, String>>.from(pData.map((p) => {
            'name': p['npay_kor_nm'],
            'min': _formatter.format(p['min_amt'] ?? 0),
            'max': _formatter.format(p['max_amt'] ?? 0),
            'info': p['description'] ?? '',
          }));
  ...
  // [실제 구현] 수정된 기본 정보를 Supabase에 업데이트하는 함수
  Future<void> _updateHospitalInfo() async {
    if (_currentYkiho == null) return;
    try {
      await supabase.from('hospitals').update({
        'yadm_nm': hospitalBaseInfo['name'],
        'addr': hospitalBaseInfo['address'],
        'operating_hours': hospitalBaseInfo['operatingHours'],
      }).eq('ykiho', _currentYkiho!);

      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('병원 정보가 저장되었습니다.')));
    } catch (e) {
      print("업데이트 에러: $e");
    }
  }
  ...
  void _searchAddress() async {
    ...
    // 데이터가 제대로 왔는지, 그리고 String 타입이 맞는지 확인
    if (result != null && result is String && result.isNotEmpty) {
      if (!mounted) return;
      ...
      // 3. DB 저장은 화면 업데이트가 완전히 끝난 뒤에!
      try {
        await _updateHospitalInfo();
      } catch (e) {
        print("저장 중 오류: $e");
      }
    }
  ...
  void _showProductTable() async {
    ...
    final result = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => ProductTablePage(
          ...
          ykiho: _currentYkiho!, // 잊지 말고 ykiho를 넘겨줘야 저장됩니다!
        ),
      ),
    );
    ...
    if (result != null) {
      ...
      // ProductTablePage 안에서 이미 저장 로직을 수행하므로 여기선 갱신만
    }
  }
        ...
        builder: (context, setSheetState) => Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              // _showDoctorSheet 함수 내부의 _saveButton 부분을 이렇게 수정하세요.
              _saveButton(() async {
                final String name = nameCtrl.text;
                final String special = specCtrl.text;
                ...
                try {
                  if (index == null) {
                    ...
                    await supabase.from('doctors').insert({
                      ...
                      'name': name,
                      'specialty': special,
                      'started_year': int.parse(year),
                      'started_month': int.parse(month),
                    });
                  } else {
                    // 2. 기존 수정
                    await supabase.from('doctors').update({
                      ...
                      'started_year': int.parse(year),
                      'started_month': int.parse(month),
                    }).eq('ykiho', _currentYkiho!).eq('name', doctors[index]['name']!);
                  }

                  // 중요: 저장이 완료된 후 데이터를 다시 불러오고, '그 다음에' 창을 닫습니다.
                  await _loadInitialData();

                  if (mounted) {
                    Navigator.pop(context); // StatefulWidget의 context 사용
                  }
                } catch (e) {
                  print("의료진 저장 오류: $e");
                }
  ...
  Widget _saveButton(Future<void> Function() onSave) { // VoidCallback 대신 Future Function으로 변경
    return SizedBox(
        width: double.infinity,
        height: 50,
        child: ElevatedButton(
            onPressed: () async { // async 추가
              await onSave(); // 데이터 저장이 완전히 끝날 때까지 기다립니다 (await)
              if (context.mounted) {
                Navigator.pop(context); // 저장이 끝난 '후에' 창을 닫습니다.
              }
            },
            style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xFF005A9C),
...
class ProductTablePage extends StatefulWidget {
  ...
  const ProductTablePage({
    ...
    required this.ykiho, // 이 줄이 없어서 에러가 난 거예요!
  });
...
class _ProductTablePageState extends State<ProductTablePage> {
  ...
  Future<void> _saveProductsToSupabase(List<Map<String, String>> products) async {
    final supabase = Supabase.instance.client;
    final String myYkiho = "병원ID_또는_ykiho"; // 현재 로그인한 병원의 ykiho

    try {
      // 1. 기존 가격 정보를 한번 지우고 새로 넣거나 (Upsert)
      // 여기서는 간단하게 리스트를 돌면서 insert 합니다.
      for (var item in products) {
        await supabase.from('hospital_prices').upsert({
          'ykiho': myYkiho,
          'npay_cd': item['name'], // 명칭을 코드로 쓰거나 별도 매핑
          'npay_kor_nm': item['name'],
          'min_amt': int.tryParse(item['min']?.replaceAll(',', '') ?? '0'),
          'max_amt': int.tryParse(item['max']?.replaceAll(',', '') ?? '0'),
  ...
  void _saveAndExit() async {
    ...
    await _saveProductsToSupabase(results);
